...
while getopts ":u:d:q:g:n:h" opt; do
        case $opt in
                ...
                g)
                        genomespath="$OPTARG"
                        ;;
                h)      echo "usage:  ./RADS.sh -u [upstream integer (optional, default = 5000)] -d [downstream integer(optional, default = 5000)] -q [fasta query>
                        echo ""
                        echo "[REQUIRED OPTIONS]"
                        echo "-------------------------------------------------------------"
                        echo "-g        path to directory containing directories with genomes information. Each genome should have its own directory containing .f>
                        echo ""
                ...
                n)
                        ...
                        ;;
                \?)
                        echo "option -$OPTARG requires an argument. Usage: bash ./RADS.sh -u [upstream integer (optional, default = 5000)] -d [downstream integer(>
                        exit 1
                        ;;
                :)
                        echo "option -$OPTARG requires an argument. Usage: bash ./RADS.sh -u [upstream integer (optional, default = 5000)] -d [downstream integer(>
...
if [ ! -f ../blast_results_30_$samplename/EFB0058_blast_${i}.txt ]; then
        diamond blastp -d ${i} --query ../$query --threads 40 --out ../blast_results_30_$samplename/EFB0058_blast_${i}.txt --outfmt 6 qseqid sseqid length nident >
fi
done
cd ../
...
do
  if [ -s blast_results_30_$samplename/EFB0058_blast_${i}translated.faa.db.dmnd.txt ]; then
    ...
    seqkit grep -f EFB0058_ORFS_$samplename/${i}_EFB0058_ORFs.txt genomes_translated_/${i}translated.faa | seqkit seq -n > EFB0058_ORFS_$samplename/${i}_EFB0058_h>
    echo "Coordinates extracted"
    awk 'BEGIN{OFS="\t"} {F="#"} {up=$3; down=$5; if (up>down) print down, up; else print up, down}' EFB0058_ORFS_$samplename/${i}_EFB0058_hits_coordinates.txt > >
    awk -v upint=$upint -v downint=$downint 'BEGIN{OFS="\t"} {up=$1-upint; down=$2+downint; if (up>0) print up, down; else print 0, down}' EFB0058_formatted_coord>
    ...
    paste EFB0058_flanks_$samplename/${i}_EFB0058_contigs.txt EFB0058_final_coordinates_$samplename/${i}_EFB0058_final_coordinates.txt > bedfiles_$samplename/${i}>
    echo "Bed file created"
    seqkit subseq --bed bedfiles_$samplename/${i}.bed genomes_/${i} > contigs_$samplename/${i}_EFB0058_contigs.fna
...
bash ../011_RADSPipelinePerfection/my_interproscan/interproscan-5.69-101.0/interproscan.sh -i interposcaninput_$samplename.faa
